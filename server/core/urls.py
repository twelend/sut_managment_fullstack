...
from .views import (
    LessonApplicationCreateView,
    ContestApplicationCreateView,
    SubjectListForTeacherView,
    SubjectCreateView,
    SubjectDeleteView,
    SubjectStatusUpdateView,
    ContestListForTeacherView,
    LessonApplicationsListView,
    ContestApplicationsListView,
    ApplicationActionView,
    StatisticsView,
    LoginView,
    ContestCreateView,
    ContestDeleteView,
    RefbookTemplateListView,
    RefbookTemplateCreateView,
    RefbookTemplateDeleteView,
    PublicSubjectListView,
    PublicContestListView,
)

urlpatterns = [
    path("teacher/login/", LoginView.as_view(), name="login"),

    # Публичные поинты для лендинга (без авторизации)
    path("landing/subjects/", PublicSubjectListView.as_view(), name="landing-subjects"),
    path("landing/contests/", PublicContestListView.as_view(), name="landing-contests"),
    ...
    path("teacher/subjects/update/<int:pk>/", SubjectStatusUpdateView.as_view(), name="teacher-subject-update"),
    ...
    path(
        "teacher/subject/<int:subject_id>/applications/",
        LessonApplicationsListView.as_view(),
        name="subject-apps",
    ),
    path(
        "teacher/contest/<int:contest_id>/applications/",
        ContestApplicationsListView.as_view(),
        name="contest-apps",
    ),

    path(
        "teacher/application/<str:app_type>/<int:pk>/action/",
